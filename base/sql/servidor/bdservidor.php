...
class bdservidor{
	...
	function select_dadosSolicitacao($seq_solicitacao=""){
		try {
			$sql 	= "	SELECT *						
						FROM tbl_solicitacao s
						WHERE s.seq_solicitacao='".$seq_solicitacao."' or '-1'='".$seq_solicitacao."';";
			//echo $sql;							
	...
	function select_dadosSO(){
		try {
			$sql 	= "	SELECT *						
						FROM aux_sistema_operacional so
						WHERE so.flg_sistema_ativo='S';";
			//echo $sql;							
			$q = $this->db->execute($sql);		
			$res = $q->getRows();
			return $res;				
